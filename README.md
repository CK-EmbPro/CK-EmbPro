# Cyiza Kenny Debrice

### Full-Stack Developer | React.js | Next.js | NestJS | Node.js | Express.js | Spring Boot | React Native

---

👋 Hi! I'm Cyiza Kenny Debrice, a passionate full-stack developer with experience building modern web and mobile applications. I enjoy solving problems and creating seamless user experiences.

---

### 🔧 Technologies & Tools

- **Programming languages:** Javascript, Typescript, Java, Html , Python, C, C++
- **Frontend:** React.js, Next.js, React Native, HTML , CSS, Tailwindcss, Bootstrapcss
- **Backend:** Node.js, Express.js, NestJS, Spring Boot
- **Databases:** MongoDB, MySQL, PostgreSQL
- **Version Control:** Git, GitHub
- **Others:** Docker, Kubernetes, CI/CD, REST APIs, Prisma
- **Technical documentation** Api documentation(Postman, Insomnia), Diagramming and user manuals(Miro), Jira , Trello

---

### 📚 Skills Overview

- **Frontend Development:** Building responsive and dynamic user interfaces using React.js, Next.js, and React Native for mobile.
- **Backend Development:** Developing RESTful APIs and backend systems with Node.js, Express.js, and NestJS.
- **Mobile App Development:** Developing interactive, fully-functional and cross-platform mobile applications using flutter and react native
- **Full-Stack Experience:** Handling complex integrations, including working with Spring Boot for robust enterprise applications.
- **Cloud & DevOps:** Experience with Docker, Kubernetes, and deploying applications on cloud platforms.

---

### 🚀 My Projects

- **Project 1:** [My Portfolio](https://iamck.netlify.app/)
  - Personal website for showcasing my professional profile.

- **Project 2:** [Trust me](https://trustme.rw/)
  - TrustMe - Trust Me | Debt Management System Rwanda | Digital ID Platform(in production already)
    
- **Project 3:** [Stockzz](https://stockzz.netlify.app/)
  - An all-in-on stock management system
 
- **Project 5:** [Car deal](https://cardealfe.netlify.app/)
  - E-commerce system for making the right deals for buying and renting cars
  
- **Project 6:** [Mk Scholar Driving school](https://github.com/CK-EmbPro/EmployeeMS-SB-RESTApis.git)
  - MK Scholar Driving School – interactive quizzes, permit test prep, and expert driving tips.

- **Project 4:** [Random Quote Generator](https://randomquotgen.netlify.app/)
  - A random quote generator

- **Project 7:** [Text-Analyzer tool](https://textanalyzzrer.netlify.app/)
  - A react-based tool for counting words , spaces, characters and estimating reading time
  
- **Project 8:** [Password generator](https://pwordgentool.vercel.app/)
  - A weak, medium and strong password generator
 
- **Project 9:** [Canalife](https://canalife2.vercel.app/)
  - A homepage for Canalife project
 
- **Project 10:** [Student_Management_system](https://github.com/CK-EmbPro/StudentMS-SB.git)
  - Springboot RestFul APIs for student management system
    
- **Project 11:** [Employee_Management_system ](https://github.com/CK-EmbPro/EmployeeMS-SB-RESTApis.git)
  - Springboot RestFul APIs for employee management system

- **Project 12:** [File_manage](https://github.com/CK-EmbPro/file_manage.git)
  - Uploading , downloading and listing files on a directory in the remote server
---


[![CK-EmbPro's GitHub stats](https://github-readme-stats.vercel.app/api?username=CK-EmbPro)](https://github.com/anuraghazra/github-readme-stats)

---

---

### 📫 Contact Me

- **Email:** kennydebrice2@gmail.com
- **LinkedIn:** [Cyiza Kenny Debrice](https://www.linkedin.com/in/cyiza-kenny-debrice-544091260/)
- **GitHub:** [@CK-EmbPro](https://github.com/CK-EmbPro)
- **Instagram:** [@ck_the_1st](https://instagram.com/ck_the_1st)
- **Phone number:** +250798594425

---



